package com.quiz.lesson06.bo;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.web.bind.annotation.RequestParam;

import com.quiz.lesson06.dao.BookingDAO;
import com.quiz.lesson06.domain.Booking;

@Service
public class BookingBO {
	@Autowired
	private BookingDAO dao;
	// input:X    output:List<Booking>
	public List<Booking> getBookingList(){
		return dao.selectBookingList();
	}
	
	// input:id    output:X
	public void deleteBookingById(int id) {
		dao.deleteBookingById(id);
	}
	
	public void addBooking(String name, String date, int day, int headcount, String phoneNumber) {
		dao.insertBooking(name, date, day, headcount, phoneNumber);
	}
	
	// input:name, phoneNumber   output: null or Booking
	public Booking getBookingByNamePhoneNumber(String name, String phoneNumber) {
		List<Booking> bookingList = dao.selectBookingByNamePhoneNumber(name, phoneNumber);
		// 0 1(최신)
		// 리스트가 비어있으면 null이 아닌 []
		if(bookingList.isEmpty()) {
			return null;
		}
		
		// 리스트가 비어있지 않으면 마지막 객체 리턴
		return bookingList.get(bookingList.size() - 1); // Booking
	}
}
