			...
			<section class="weather-history col-10 mt-3 ml-5">
				...
				<table class="table text-center">
					...
					<c:forEach items="${list}" var="lists" varStatus="status">
						<tr>
							<td><fmt:formatDate value="${lists.date}" pattern="yyyy년 M월 d일" /></td>
							<td>
								<c:choose>
									<c:when test="${lists.weather eq '흐림'}">
										<img src="/img/cloudy.jpg">
									</c:when>
									<c:when test="${lists.weather eq '구름조금'}">
										<img src="/img/partlyCloudy.jpg">
									</c:when>
									<c:when test="${lists.weather eq '비'}">
							...
							<td>${lists.temperatures}${temp}</td>
							<td>${lists.precipitation}</td>
							<td>${lists.microDust}</td>
							<td>${lists.windSpeed}</td>
						</tr>
					</c:forEach>
					</tbody>
